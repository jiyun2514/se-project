...
|1.1| 2026.05.13 | 유지윤 | 진행 상황 관리 및 형상 관리 내용 보완 | 수정 |
...
| 리스크 | 영향 | 대응 방안 |
|----------|----------|----------|
| 일정 지연 | 프로젝트 완료 시기 지연 | 기능 우선순위를 재조정하고 주간 진행 상황을 점검한다. |
| 기능 구현 실패 | 일부 기능 미완성 가능성 | 핵심 기능 중심으로 범위를 조정하고 대체 기능을 설계한다. |
| Git 충돌 및 형상 관리 문제 | 작업 내용 손실 및 개발 지연 | 브랜치 전략과 Pull Request를 활용하여 변경 사항을 관리한다. |
| 요구사항 변경 | 문서 및 설계 수정 필요 | 변경 이력을 기록하고 관련 문서를 즉시 갱신한다. |
| 테스트 단계 오류 발견 | 배포 일정 지연 | 기능 구현 후 단위 테스트 및 통합 테스트를 수행한다. |
| 외부 데이터/API 사용 불가 | 일부 기능 동작 불가 | 대체 데이터 또는 Mock 데이터를 활용하여 개발을 진행한다. |
